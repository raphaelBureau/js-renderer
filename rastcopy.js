...
export class Rasterizer {
    ...
    DrawTriangle(face, profiler) {
        if (face[2].wValues[face[0][1]] < 0 && face[2].wValues[face[0][2]] < 0 && face[2].wValues[face[0][3]] < 0) {
            return;
        }
        let v1x = face[0][0][face[0][1] * 4];
        let v1y = face[0][0][face[0][1] * 4 + 1];
        let v1z = face[0][0][face[0][1] * 4 + 2];
        let v2x = face[0][0][face[0][2] * 4];
        let v2y = face[0][0][face[0][2] * 4 + 1];
        let v2z = face[0][0][face[0][2] * 4 + 2];
        let v3x = face[0][0][face[0][3] * 4];
        let v3y = face[0][0][face[0][3] * 4 + 1];
        let v3z = face[0][0][face[0][3] * 4 + 2];
        let rgba = face[2];
        //bounding box
        ...
        let bestZ = Math.min(v1z, v2z, v3z);
        let farthestZ = Math.max(v1z, v2z, v3z);
        ...
        let InTri = false;
        let v0 = [v2x - v1x, v2y - v1y];
        let v1 = [v3x - v1x, v3y - v1y];
        let DBI = 0;
        ...
        let vertXv32y = v32y * v2x;
        let vertXv13y = v13y * v3x;
        let vertXv21y = v21y * v1x;
        ...
        let preAlpha2 = 0;
        let dv11 = denom * v1[1];
        let alphaVDX = - dv11 * v1x;
        let preBeta1 = 0;
        let dv01 = denom * v0[1];
        let betaVDX = dv01 * v1x;
        ...
        for (let y = minY; y <= maxY; y++) {//optimisations possibles GENRE BEAUCOUP
            ...
            preAlpha2 = - v1[0] * (y - v1y) * denom + alphaVDX;
            preBeta1 = v0[0] * (y - v1y) * denom + betaVDX;
            ...
            for (let x = minX; x <= maxX; x++) {
                ...
                if (depthValue == 0 || bestZ < depthValue) {//si le cote le plus proche du triangle est plus loin que le point ecris au buffer, skip
                    if (
                        ...
                        alpha = x * dv11 + preAlpha2;
                        beta = preBeta1 - x * dv01;
                        gamma = 1 - alpha - beta;
                        depth = gamma * v1z + alpha * v2z + beta * v3z;
